...
// 引用config文件，以获取要打包的项目目录。
var cfg = require('./config.json');
var project = process.argv[2];
var cfg = cfg[project];

// 编译的项目文件会对应生成在一个build目录下。
var dest = "build/" + project + "/";
//静态资源根目录
var resource = ".." + cfg.resource;
//源模板文件目录f
var srcHtml = resource;
//编译后模板文件目录，可供测试和上线。
var destHtml = resource.replace("www_src", "php");
var imgSource = resource + "img/";
var defineSuffix = cfg.suffix;
var onlyHtml = false;

//加载插件，安装后的node_modules插件，放置的位置要与项目相分离。
...
// through2：Node Stream.Transform 流操作的简单封装。
...
function uploadCdn(file) {
  return new Promise(function(resolve, reject) {
    ...
    // post的方式上传本地文件。
    request.post({
      url: 'http://upload.static.huajiao.com/file/upload',
      formData: formData
...
gulp.task("revreplace", ["rev"], function() {
  ...
  return gulp.src([dest + "/**"])
    .pipe(revReplace({
      manifest: manifest,
      replaceInExtensions: ['.html', '.tpl', '.phtml', '.css', ".js"]
    }))
